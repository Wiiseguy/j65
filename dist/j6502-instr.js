...
exports.IND_Y_MATCH = exports.IND_X_MATCH = exports.IND_MATCH = exports.ABS_Y_MATCH = exports.ABS_X_MATCH = exports.ABS_MATCH = exports.ZP_Y_MATCH = exports.ZP_X_MATCH = exports.ZP_MATCH = exports.REL_MATCH = exports.IMM_MATCH = exports.InstructionsArray = exports.InstructionsByOp = exports.Instructions = void 0;
// Addressing mode matchess
const IMM_MATCH = /^#(\$[0-9a-f]{1,2}|%[0-1]{1,8}|[0-9]{1,3})$/;
exports.IMM_MATCH = IMM_MATCH;
const REL_MATCH = /^([a-z_]+[a-z0-9_]*|\$[0-9a-f]{1,2})$/; // e.g. label_name or $F9
exports.REL_MATCH = REL_MATCH;
const ZP_MATCH = /^\$[0-9a-f]{1,2}$/;
exports.ZP_MATCH = ZP_MATCH;
const ZP_X_MATCH = /^\$[0-9a-f]{1,2},x$/;
exports.ZP_X_MATCH = ZP_X_MATCH;
const ZP_Y_MATCH = /^\$[0-9a-f]{1,2},y$/;
exports.ZP_Y_MATCH = ZP_Y_MATCH;
const ABS_MATCH = /^([a-z_]+[a-z0-9_]*|\$[0-9a-f]{3,4}$)/;
exports.ABS_MATCH = ABS_MATCH;
const ABS_X_MATCH = /^\$[0-9a-f]{3,4},x$/;
exports.ABS_X_MATCH = ABS_X_MATCH;
const ABS_Y_MATCH = /^\$[0-9a-f]{3,4},y$/;
exports.ABS_Y_MATCH = ABS_Y_MATCH;
const IND_MATCH = /^\(\$[0-9a-f]{1,4}\)$/;
exports.IND_MATCH = IND_MATCH;
const IND_X_MATCH = /^\(\$[0-9a-f]{1,2},x\)$/;
exports.IND_X_MATCH = IND_X_MATCH;
const IND_Y_MATCH = /^\(\$[0-9a-f]{1,2}\),y$/;
exports.IND_Y_MATCH = IND_Y_MATCH;
const instructions = {
    // 0
    'BRK': { op: 0x00, size: 1, asm: 'brk', match: null, micro(m) { m.halt(); }, desc: 'Break' },
    ...
    'ASL': { op: 0x0a, size: 1, asm: 'asl', match: null, micro(m) { m.C = (m.CPU.A & 0b1000_0000) >> 7; m.NZ = m.A = m.CPU.A * 2; /* a << 1 === a * 2 */ }, desc: 'Shift Left One Bit (Accumulator)' },
    // Unused: 0x0b - 0x0c
    ...
    // Unused: 0x1f
    // 2
    'JSR_ABS': { op: 0x20, size: 3, asm: 'jsr', match: ABS_MATCH, micro(m) { m.AD = m.fetch16(); m.PC = m.PC - 1; m.push(m.PCH); m.push(m.PCL); m.PC = m.AD; }, desc: 'Jump to New Location Saving Return Address' },
    ...
    'ROL_ABS': { op: 0x2e, size: 3, asm: 'rol', match: ABS_MATCH, micro(m) { m.ADR = m.fetch16(); m.AD = m.read(m.ADR) << 1; m.ADL = m.ADL | m.C; m.NZ = m.ADL; m.C = m.ADH; m.write(m.ADR, m.ADL); }, desc: 'Rotate One Bit Left (Absolute)' },
    // Unused: 0x2f
    // 3
    ...
    // Unused: 0x37
    'SEC': { op: 0x38, size: 1, asm: 'sec', match: null, micro(m) { m.C = 1; }, desc: 'Set Carry Flag' },
    ...
    'LSR': { op: 0x4a, size: 1, asm: 'lsr', micro(m) { m.C = m.CPU.A; m.NZ = m.A = m.CPU.A >> 1; }, match: null, desc: 'Shift One Bit Right (Accumulator)' },
    // Unused: 0x4b
    'JMP_ABS': { op: 0x4c, size: 3, asm: 'jmp', micro(m) { m.AD = m.fetch16(); m.PC = m.AD; m.halt(); }, match: ABS_MATCH, desc: 'Jump to New Location (Absolute)' },
    ...
    // Unused: 0x8b
    'STY_ABS': { op: 0x8c, size: 3, asm: 'sty', match: ABS_MATCH, micro(m) { m.write(m.fetch16(), m.CPU.Y); }, desc: 'Store Index Y in Memory (Absolute)' },
    'STA_ABS': { op: 0x8d, size: 3, asm: 'sta', match: ABS_MATCH, micro(m) { m.write(m.fetch16(), m.CPU.A); }, desc: 'Store Accumulator in Memory (Absolute)' },
    'STX_ABS': { op: 0x8e, size: 3, asm: 'stx', match: ABS_MATCH, micro(m) { m.write(m.fetch16(), m.CPU.X); }, desc: 'Store Index X in Memory (Absolute)' },
    // Unused: 0x8f
    // 9
    ...
    'TXS': { op: 0x9a, size: 1, asm: 'txs', match: null, micro(m) { m.SP = m.CPU.X; }, desc: 'Transfer Index X to Stack Register (SP = X)' },
    // Unused: 0x9b - 0x9c
    'STA_ABS_X': { op: 0x9d, size: 3, asm: 'sta', match: ABS_X_MATCH, micro(m) { m.write(m.fetch16() + m.CPU.X, m.CPU.A); }, desc: 'Store Accumulator in Memory (Absolute,X)' },
    // A
    ...
    'LDA_ABS_Y': { op: 0xb9, size: 3, asm: 'lda', match: ABS_Y_MATCH, micro(m) { m.NZ = m.A = m.read(m.fetch16() + m.CPU.Y); }, desc: 'Load Accumulator with Memory (Absolute,Y)' },
    ...
    'LDA_ABS_X': { op: 0xbd, size: 3, asm: 'lda', match: ABS_X_MATCH, micro(m) { m.NZ = m.A = m.read(m.fetch16() + m.CPU.X); }, desc: 'Load Accumulator with Memory (Absolute,X)' },
    'LDX_ABS_Y': { op: 0xbe, size: 3, asm: 'ldx', match: ABS_Y_MATCH, desc: 'Load Index X with Memory (Absolute,Y)' },
    // C
    'CPY_IMM': { op: 0xc0, size: 2, asm: 'cpy', match: IMM_MATCH, micro(m) { m.SIR = m.CPU.Y - m.fetch(); m.C = m.SIR >= 0; m.NZ = m.SIR; }, desc: 'Compare Memory with Index Y (Immediate)' },
    ...
    // Unused: 0xc7
    'INY': { op: 0xc8, size: 1, asm: 'iny', match: null, micro(m) { m.NZ = m.Y = m.CPU.Y + 1; }, desc: 'Increment Index Y by One' },
    'CMP_IMM': { op: 0xc9, size: 2, asm: 'cmp', match: IMM_MATCH, desc: 'Compare Memory with Accumulator (Immediate)' },
    'DEX': { op: 0xca, size: 1, asm: 'dex', match: null, micro(m) { m.NZ = m.X = m.CPU.X - 1; }, desc: 'Decrement Index X by One' },
    // Unused: 0xcb
    ...
    // Unused: 0xdf
    // E
    'CPX_IMM': { op: 0xe0, size: 2, asm: 'cpx', match: IMM_MATCH, micro(m) { m.SIR = m.CPU.X - m.fetch(); m.C = m.SIR >= 0; m.NZ = m.SIR; }, desc: 'Compare value and Index X (Immediate)' },
    ...
    // Unused: 0xe7
    'INX': { op: 0xe8, size: 1, asm: 'inx', match: null, micro(m) { m._X(m.CPU.X + 1); m._NZ(m.CPU.X); }, desc: 'Increment Index X by One' },
    ...
    // Unused: 0xeb
    'CPX_ABS': { op: 0xec, size: 3, asm: 'cpx', match: ABS_MATCH, micro(m) { m.SIR = m.CPU.X - m.read(m.fetch16()); m.C = m.SIR >= 0; m.NZ = m.CPU.X; }, desc: 'Compare value at address and Index X (Absolute)' },
...
exports.InstructionsByOp = InstructionsByOp;
let InstructionsArray = [];
exports.InstructionsArray = InstructionsArray;
