...
class J6502_Program {
    ...
    getLabelAddress(name) {
        let label = this.labels[name];
        if (label === undefined)
            throw new RangeError(`Unknown label: ${name}`);
        //console.log("Fetched label:", name, '=', label.toString(16));
        return label;
    }
    getRelativeLabelAddress(name, pos) {
    ...
    build(clamp = false) {
        ...
        for (let a of this.assembly) {
            if (a instanceof Instruction) {
                let instr = j6502_instr_1.Instructions[a.name];
                let data;
    ...
    add(name, data = undefined) {
        let instr = j6502_instr_1.Instructions[name];
        if (!instr)
            throw RangeError(`Unknown instruction: ${name}`);
    ...
    setLabel(name) {
        if (this.labels[name] !== undefined)
            throw new RangeError(`Label redefinition: ${name}`);
    ...
    writeFile(fileName) {
        ...
        (0, fs_1.writeFileSync)(fileName, prgBuf);
    }
}
exports.J6502_Program = J6502_Program;
